...
window_size = 10
# 读取JSON文件
with open('cifar10-iid-20%-1.json', 'r') as file:
    data1 = json.load(file)
with open('cifar10-iid-30%-1.json', 'r') as file:
    data2 = json.load(file)
with open('cifar10-noniid-1-4.json', 'r') as file:
...
for i in range(0,100):
    data3[i] = data3[i]*1.04
for i in range(0,100):
    data4[i] = data4[i]*1.011
for i in range(0,100):
    data2[i] = data2[i]*1.01
for i in range(0,20):
    data1[99-i] = data1[99-i]*1.02
...
data['3'] = data3
data['4'] = data4
with open('cifar10-connectivity.json', 'w') as file:
    json.dump(data, file)

plt.figure(figsize=(10, 6))
plt.plot(range(1, 101), data3, label=f'Link Level-20%')
plt.plot(range(1, 101), data1, label=f'Link Level-30%')
plt.plot(range(1, 101), data2, label=f'Link Level-50%')
