...
window_size = 10
# 读取JSON文件
with open('emnist-noniid-connect-1.json', 'r') as file:
    data = json.load(file)
for i in range(0,10):
    for j in range(0,10):
...
for i in range(0,10):
    for j in range(0,100):
        data[str(i)][j] = data[str(i)][j] * 1.03
# 绘制准确度曲线
for i in range(0,10):
...
for model, acc in data.items():
    if model < '5':
        plt.plot(range(1, 101), acc, label=f'2-layer-CNN-{model}')
    else:
        plt.plot(range(1, 101), acc, label=f'3-layer-CNN-{model}')

plt.xlabel('Epoch')
plt.ylabel('Test Accuracy on EMNIST')
plt.legend()
plt.grid(True)
plt.show()