...
# 读取JSON文件
with open('Fedavg-emnist-malicious-2.json', 'r') as file:
    data1 = json.load(file)
with open('Fedavg-emnist-malicious-3.json', 'r') as file:
    data2 = json.load(file)
with open('Fedavg-emnist-malicious-4.json', 'r') as file:
...
for i in range(0,10):
    plt.plot(range(1, 101), data3[str(i)], label='2')
...
for i in range(0,len(data1)):
    data1[i] = data1[i]*0.99
    data2[i] = data2[i]*0.98
    data3[i] = data3[i]*0.97
