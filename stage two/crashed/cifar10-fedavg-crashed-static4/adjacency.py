...
def dynamic(adjacency_matrix,radius):
    num_nodes = len(adjacency_matrix)
    near_adjacency = []
    for i in range(num_nodes):
        near = []
        for j in range(num_nodes):
            if adjacency_matrix[i][j] < radius:
                near.append(j)
        near_adjacency.append(near)
    return near_adjacency


def disconnect(node, adjacency_matrix):
    """
    断开一个节点与其它节点的连接
    ...
    Returns:
        numpy.ndarray: 断开连接后的邻接矩阵
    """
    adjacency_matrix[node, :] = 100
    adjacency_matrix[:, node] = 100
    adjacency_matrix[node, node] = 0
    return adjacency_matrix
