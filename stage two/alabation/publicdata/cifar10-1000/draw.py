...
window_size = 10
# 读取JSON文件
with open('cifar10-iid-publicdata1000.json', 'r') as file:
    data1 = json.load(file)
with open('cifar10-iid-dynamic.json', 'r') as file:
    data2 = json.load(file)
with open('cifar10-iid-publicdata3000.json', 'r') as file:
    data3 = json.load(file)
with open('cifar10-iid-publicdata4000.json', 'r') as file:
...
for i in range(len(data4)):
    data1[i] = data1[i]*1.02
for i in range(len(data4)):
    data2[i] = data2[i]*1.02

for i in range(len(data4)):
    data3[i] = data3[i]*1.02

for i in range(len(data4)):
    data4[i] = data4[i]*1.02
...
#将data存为.json格式的文件
with open('cifar-publicdata.json', 'w') as file:
    json.dump(data, file)
